...
const Profile = ({setMessage, setMessageStyle}) => {
  ...
  const updateUserInfo = async () => {
    if (name == "" || email == "") {
      setMessage("Following fields cannot be empty: First Name, Email Address");
      ...
      await UserService.update(user.id, name, email, gender, dateOfBirth, phoneNumber, photo, user.address ? user.address.id : null, street, city, zipCode, state, country, user.token);
      if (user.email !== email) {
        AuthService.logout();
